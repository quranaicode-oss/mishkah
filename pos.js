...
<html lang="${lang}" dir="${dirAttr}">
  ...
  function createCentralPosSync(options={}){
    ...
    const extractAllowedMethods = (res)=>{
      if(!res || typeof res.headers?.get !== 'function') return [];
      const raw = res.headers.get('allow');
      if(!raw) return [];
    ...
    const fetchSnapshot = async ()=>{
      ...
      for(let attempt = 0; attempt < httpCandidates.length; attempt += 1){
        ...
        emitDiagnostic('http:fetch:start', {
          ...
          data:{ endpoint: endpointCandidate, attempt: attempt + 1, method: httpMethod }
        });
        try{
          response = await fetch(endpointCandidate, { headers, method: httpMethod });
        } catch(fetchError){
        ...
        const allowedMethods = extractAllowedMethods(response);
        if((httpStatus === 404 || httpStatus === 405 || httpStatus === 410) && attempt < httpCandidates.length - 1){
          emitDiagnostic('http:fetch:fallback', {
            ...
            data:{ endpoint: endpointCandidate, httpStatus, method: httpMethod, next: httpCandidates[attempt + 1] || null, allow: allowedMethods }
          });
          continue;
        }
        break;
      ...
      const statusNumber = Number(response.status);
      httpStatus = Number.isFinite(statusNumber) ? statusNumber : response.status;
      const finalAllowedMethods = extractAllowedMethods(response);
      ...
      if(httpStatus === 401 || httpStatus === 403){
        emitDiagnostic('http:fetch:unauthorized', {
          ...
          data:{ endpoint: lastEndpoint, httpStatus, method: httpMethod, allow: finalAllowedMethods, authOff }
        });
        if(authOff){
      ...
      if(httpStatus === 404 || httpStatus === 405 || httpStatus === 410){
        ...
        emitDiagnostic(requireOnline && !allowLocalFallback ? 'config:disable' : 'http:fetch:fallback', {
          ...
          data:{ ...offlineData, method: httpMethod, allow: finalAllowedMethods, allowLocalFallback }
        });
        if(requireOnline && !allowLocalFallback){
      ...
      if(!response.ok){
        emitDiagnostic('http:fetch:error', {
          ...
          data:{ endpoint: lastEndpoint, httpStatus, method: httpMethod, allow: finalAllowedMethods }
        });
      ...
      emitDiagnostic('http:fetch:success', {
        ...
        data:{ endpoint: lastEndpoint, httpStatus, method: httpMethod, allow: finalAllowedMethods, version: body?.version ?? null, hasSnapshot: !!body?.snapshot }
      });
