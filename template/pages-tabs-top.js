(function (window) {
  ...
  function PagesTabsTopRender(db) {
    ...
    const tabsStripe = D.Containers.Div({
      attrs: { class: tw`flex flex-wrap items-center justify-center gap-2 rounded-3xl border border-[color-mix(in_oklab,var(--border)50%,transparent)] bg-[color-mix(in_oklab,var(--surface-1)95%,transparent)] p-3 shadow-[0_18px_36px_-28px_rgba(15,23,42,0.4)]` }
    }, navButtons);

    const contentCard = D.Containers.Section({
      attrs: { class: tw`space-y-4 rounded-3xl border border-[color-mix(in_oklab,var(--border)55%,transparent)] bg-[color-mix(in_oklab,var(--surface-1)96%,transparent)] p-6 shadow-[0_26px_56px_-30px_rgba(15,23,42,0.48)]` }
    }, [
    ...
    return D.Containers.Main({
      attrs: { class: tw`mx-auto flex w-full max-w-4xl flex-col gap-5 px-4 py-6` }
    }, [tabsStripe, contentCard]);
  }
